...
`tts-stt` is a small, fully offline speech app:

- **Backend/UI**: FastAPI app in `app/main.py`, single-page UI in `app/static/index.html`.
- **TTS**: Piper (`app/tts.py`) — voice `en_US-lessac-medium`.
- **STT**: Vosk (`app/stt.py`) — model `vosk-model-small-en-us-0.15`; audio is normalized with `ffmpeg` to 16 kHz mono before recognition.
